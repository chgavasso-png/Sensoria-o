...
function editarBrinquedo(id) {
    ...
    document.getElementById('titulo-form-brinqueto').textContent = 'Editar Brinquedo';
    window.scrollTo({ top: 0, behavior: 'smooth' });
}
...
function atualizarPreviaQrSite() {
    const url = document.getElementById('cfg-site-qrcode').value.trim();
    const siteUrl = 'https://aumapiracicaba.org.br/sensoriacao';
    const img = document.getElementById('cfg-site-qr-previa');
    if (img) {
        img.src = url
            ? url
            : 'https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' + encodeURIComponent(siteUrl);
    }
}
